...
Create a `.dev.vars` file in the root directory (used by Wrangler in local dev).

**Default Claude models** use **Amazon Bedrock** (same idea as the Python `app/` stack: region + Bedrock model / inference profile ID). Set:
...
Your IAM principal needs permission to invoke the configured models (for example `bedrock:InvokeModel` on the ARNs you use, or a policy that covers inference profiles in that region). In the AWS console, enable access to the specific Bedrock foundation models you rely on.

Optional: direct API keys if you switch entries in `shared/models.ts` back to non-Bedrock providers:
...
Bedrock model IDs are region-prefixed (for example `eu.anthropic.claude-sonnet-4-6`). Edit `AGENT_MODEL_DEFINITIONS` in `shared/models.ts` if your account uses different regions or inference profile IDs.
...
Use `provider: 'anthropic'`, `'openai'`, or `'google'` with the corresponding API key in `.dev.vars` if you are not using Bedrock. New providers may need wiring in `worker/do/AgentService.ts`.
